// dfc:import-runs — import the local `.agent-runs` buffer into SurrealDB memory.
//
//   pnpm dfc:import-runs --agent claude --dry-run        # parse + map, write nothing
//   pnpm dfc:import-runs --agent claude                  # import to SurrealDB
//   pnpm dfc:import-runs --agent codex --session <id>     # one session only
//   pnpm dfc:import-runs --agent claude --limit 50        # cap tool events
//   pnpm dfc:import-runs --agent claude --dry-run --json   # emit the full plan
//
...
function printSummary(plan: ImportPlan, dryRun: boolean): void {
  const { stats } = plan;
  console.log(`dfc:import-runs ${dryRun ? "(DRY RUN — no writes)" : ""}`.trim());
...
async function main(): Promise<void> {
  ...
  if (total === 0) {
    console.log("dfc:import-runs — nothing to import (no .agent-runs activity found).");
    return;
  }

// dfc:log-tool — append one tool_event to the local `.agent-runs` buffer.
//
//   pnpm dfc:log-tool --session s1 --agent codex --tool Bash --command "pnpm test"
//   pnpm dfc:log-tool --session s1 --tool mcp__github__get_pr --mcp-server github --mcp-tool get_pr
//   pnpm dfc:log-tool --session s1 --tool Write --file src/x.ts --dry-run
//
// Writes a JSONL line shape-compatible with hooks/log-agent-run.sh, so the same
// dfc:import-runs pass ingests Claude Code hook output AND manual/Codex events.
...
function main(): void {
  ...
  if (args["dry-run"] === "true") {
    console.log("dfc:log-tool (DRY RUN — not written):");
    console.log(json);
    return;
  }
  ...
  console.log(`dfc:log-tool — appended ${tool} to ${slog}`);
}

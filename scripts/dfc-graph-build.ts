// dfc:graph:build - build the repo graph DIRECTLY into SurrealDB using the Rust
// graphify-surreal binary (no python graphify → graph.json → dfc:graph:import chain).
//
//   pnpm dfc:graph:build [--repo-root /path/to/repo] [--deep] [--status|--query "q"]
//
// The binary writes graph_node/graph_edge/graph_snapshot rows in the SAME shape the
// TS readers (dfc:context, dfc:graph:query, dashboard) already consume, into the
...
function main(): void {
  ...
  if (!cfg.url || !cfg.username || !cfg.password || /<[^>]+>/.test(cfg.url + cfg.username + cfg.password)) {
    console.error("No usable SurrealDB credentials for this repo (.dfc/surreal.env). Run dfc:init / db:check first.");
    process.exit(2);
  }

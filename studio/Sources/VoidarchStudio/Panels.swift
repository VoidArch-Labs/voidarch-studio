...
// existing dashboard/Voidarch Context pages (hybrid rule from the v2 spec decisions).
import SwiftUI
import WebKit
import SwiftTerm
...
/// standalone Voidarch Context page server, which the daemon does not serve — native replaces it
        ...
        VStack(spacing: 0) {
            ...
            if let pack {
                ...
                ContentUnavailableView("Generate a context pack", systemImage: "doc.text.magnifyingglass",
                                       description: Text("Built by Voidarch Context via the daemon /api/context endpoint. Attach it to inject as {contextPack} on the next terminal launch."))
            }
            Spacer(minLength: 0)
        }
    }

// dfc:memory - unified CRUD over the memory kinds.
//   pnpm dfc:memory add    --kind lesson --text "..." [--tags a,b] [--task goal]
//   pnpm dfc:memory add    --kind snippet --file src/x.ts --language ts --path src/x.ts
//   pnpm dfc:memory list   --kind repo_fact [--limit 20] [--json]
//   pnpm dfc:memory search --kind decision --query "surrealdb" [--limit 10]
//   pnpm dfc:memory get|update|delete --kind lesson --id lesson:abc123
//   pnpm dfc:memory list|get|delete --kind context   (context packs, read/delete only)
...
/** First sentence, capped at 140 chars (same rule as dfc:remember). */
...
async function main(): Promise<void> {
  ...
  if (kind === "context" && !CONTEXT_SUBCOMMANDS.has(sub)) {
    usage(`--kind context supports only ${[...CONTEXT_SUBCOMMANDS].join("|")} (adds happen via dfc:context)`);
  }

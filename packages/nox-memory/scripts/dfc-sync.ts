// dfc:sync - one-way copy of this repo's memory between two SurrealDB instances.
...
const HELP = `dfc:sync - one-way copy of this repo's memory between SurrealDB instances

Usage:
  pnpm dfc:sync --to <url>     push from current config DB to <url>
  pnpm dfc:sync --from <url>   pull from <url> into current config DB

// dfc:nox — minimal Nox setup/status page. Local-only, no Studio controls.
//
//   pnpm dfc:nox [--repo-root /path/to/repo] [--port 4950]
  ...
  return await withDb(async (db, c) => {
    ...
    return {
      ...
      graph: latestGraph ?? {
        ...
        status: existsSync(graphJson) ? "graphify-out/graph.json exists; import it with dfc:graph:import for DB freshness" : "not indexed",
      },
    };
  }, { repoRoot });
}
...
async function main(): Promise<void> {
  ...
  server.listen(port, "127.0.0.1", () => {
    console.log(`Nox setup/status page: http://127.0.0.1:${port}`);

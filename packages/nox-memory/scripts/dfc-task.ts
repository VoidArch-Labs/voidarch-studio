// dfc:task - task state on the existing `task` table.
//   pnpm dfc:task add --goal "Ship CRUD scripts" [--status open] [--tags a,b]
//   pnpm dfc:task list [--status open|in_progress|blocked|done] [--all] [--json]
//   pnpm dfc:task update --id task:abc --status in_progress [--goal "..."]
//   pnpm dfc:task done --id task:abc
//   pnpm dfc:task get|delete --id task:abc
...
function usage(msg: string): never {
  console.error(msg);
  console.error("usage: pnpm dfc:task <add|list|update|done|get|delete> [--goal ...] [--id ...] [--status ...]");
  process.exit(2);
}

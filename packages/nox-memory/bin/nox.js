...
// `nox models <status|install>` takes its subcommand as a bare positional
// (status/install), which resolveCommand strips into `command` (e.g.
...
Setup:
  init                                     write .nox/config.json + .gitignore entries
...
Compatibility:
  pnpm dfc:* scripts remain available (dfc:context still defaults to JSON).
`);
}
...
// Spec UX: `nox context "<task>"` / `nox remember --kind X "<text>"` take a bare
// positional instead of --task/--text, which may be followed by more --flags
// (e.g. --repo-root). Find the one arg that isn't a "--flag" and isn't the value
// immediately following a preceding "--flag", and map it onto the flag the
// underlying script expects.
const POSITIONAL_FLAG = { context: "--task", remember: "--text" };
...
// `nox context` defaults to Markdown (spec: markdown-first for the CLI bin); JSON
// stays the default for `pnpm dfc:context` so the dashboard/skills stay unchanged.
